...
| GET | `/api/execution/pending-reviews` | List verdicts awaiting human review. Sources: (1) execution gateway records in non-terminal status; (2) tracker supplement — ESCALATED/APPROVED_IF decisions with no gateway record yet (`synthetic: true` flag). Ensures decisions made before the gateway existed still surface in the queue. |
...
| POST | `/api/execution/{execution_id}/dismiss` | Human dismisses a verdict. Body: `{reviewed_by, reason}` — **`reason` is required** (400 if blank). **Phase 35A:** writes `VerdictOverride(DISMISS_APPROVED or DISMISS_ESCALATED)` to `governance-overrides`. |
...
| GET | `/api/inventory/status` | Staleness check — `{exists, refreshed_at, resource_count, type_summary, age_hours, stale}`. `stale=true` when age > `inventory_stale_hours` (default 24 h). A background `_inventory_staleness_watcher` also checks every 6 h and sends a Slack alert when `age_hours > 2 × inventory_stale_hours` (~48 h by default), deduped to once per calendar day. |
| GET | `/api/decisions/{decision_id}/playbook` | **Phase 34D — Tier 3 Playbook.** Returns a Playbook JSON (`action_type`, `resource_id`, `az_command`, `executable_args`, `rollback_command`, `expected_outcome`, `risk_level`, `estimated_duration_seconds`, `requires_downtime`, `supports_native_what_if`) for the given decision. Returns `200 + {"available": false, "reason": "..."}` when no Tier 3 template exists for the action+resource combination (Tier 1 SDK tools handle those — this is a normal business state, not an error). Returns `404` only when the decision itself is not found. Returns `500` on generation error. |
...
| GET | `/api/metrics/accuracy` | **Phase 36C — Decision accuracy metrics.** Returns confusion matrix (TP/TN/FP/FN), precision, recall, F1, and per-verdict incident rates derived from labeled decisions. Query params: `days` (default 30), `action_type` (optional filter). When no decisions are labeled yet, returns `empty_state: true` with null metrics and zero counts — the Decision Quality page renders an empty-state card in this case. |
