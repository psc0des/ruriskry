...
**Last updated:** 2026-03-10 (CORS security fix: reverted `allow_origin_regex *.azurestaticapps.net` — any Azure SWA on that shared domain would match, giving any attacker with a free Azure account a valid CORS origin; fixed architecturally instead: `azurerm_static_web_app.dashboard` moved to Stage 1 of `scripts/deploy.sh` alongside ACR, its URL read immediately from `terraform output` and patched into `terraform.tfvars` before Stage 2 runs — Container App created with correct exact `DASHBOARD_URL` from day one, no stale CORS window, no re-apply step; wire-back step removed from deploy.sh; `docs/ARCHITECTURE.md` CORS + lock notes updated)
