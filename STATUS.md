...
**Last updated:** 2026-03-10 (Infrastructure security hardening: AzAPI provider added (`azure/azapi` ~> 2.0) for Foundry project management — `create_foundry_project=true` now fully Terraform-managed via `azapi_update_resource` + `azapi_resource`; SEC-02: ACR `admin_enabled=false` — Container App pulls via Managed Identity `AcrPull` role, no credentials in tfstate; SEC-03: Key Vault `purge_protection_enabled=true`, `soft_delete_retention_days=90`; SEC-04: Cosmos DB uses Managed Identity auth, `network_acl_bypass_for_azure_services=true`; SEC-05: Foundry `local_authentication_enabled=false`; SEC-06: CORS enforced in `dashboard_api.py` via `CORSMiddleware` using `DASHBOARD_URL` env var; SEC-07: Teams webhook stored in Key Vault secret, injected via Container App secret mechanism; SEC-08: `azurerm_management_lock` (CanNotDelete) on resource group; ACR image built in Azure via `az acr build` (`terraform_data` provisioner — no Docker Desktop required); `azurerm_role_assignment.subscription_reader` grants Container App MI `Reader` at subscription scope; `backend_container_app_principal_id` added to outputs; resource group renamed `ruriskry-rg` → `ruriskry-core-rg`; `terraform destroy` requires removing RG lock first)
